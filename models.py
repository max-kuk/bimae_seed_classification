...
def identity_block_3d(input_tensor, kernel_size, filters, stage, block):
    """
    3D Identity Block for ResNet

    Args:
        input_tensor: Input tensor
        kernel_size: Kernel size for convolutions
        filters: List of filter sizes for convolutions
        stage: Stage number
        block: Block number
    ...
    conv_name_base = f"res{stage}_{block}"
    bn_name_base = f"bn{stage}_{block}"

    x = conv3d_bn(
        input_tensor, filters1, kernel_size=(1, 1, 1), name=f"{conv_name_base}_a"
    )

    x = conv3d_bn(x, filters2, kernel_size, name=f"{conv_name_base}_b")

    x = conv3d_bn(x, filters3, kernel_size=(1, 1, 1), name=f"{conv_name_base}_c")
...
def conv_block_3d(input_tensor, kernel_size, filters, stage, block, strides=(2, 2, 2)):
    """
    3D Convolutional Block for ResNet
    ...
    shortcut = conv3d_bn(
        ...
        kernel_size=(1, 1, 1),
        strides=strides,
        name=f"{conv_name_base}_downsample",
    )
...
def ResNet50_3D(input_shape=(224, 224, 224, 3), classes=1000):
    """
    3D ResNet-50 Model
    ...
    # Initial convolution
    x = conv3d_bn(inputs, 64, kernel_size=(7, 7, 7), strides=(2, 2, 2), name="conv1")
    ...
    x = identity_block_3d(
        x, kernel_size=(3, 3, 3), filters=[64, 64, 256], stage=2, block="b"
    )
    x = identity_block_3d(
        x, kernel_size=(3, 3, 3), filters=[64, 64, 256], stage=2, block="c"
    )

    # Stage 3
    x = conv_block_3d(
        x, kernel_size=(3, 3, 3), filters=[128, 128, 512], stage=3, block="a"
    )
    x = identity_block_3d(
        x, kernel_size=(3, 3, 3), filters=[128, 128, 512], stage=3, block="b"
    )
    x = identity_block_3d(
        x, kernel_size=(3, 3, 3), filters=[128, 128, 512], stage=3, block="c"
    )
    x = identity_block_3d(
        x, kernel_size=(3, 3, 3), filters=[128, 128, 512], stage=3, block="d"
    )

    # Stage 4
    x = conv_block_3d(
        x, kernel_size=(3, 3, 3), filters=[256, 256, 1024], stage=4, block="a"
    )
    x = identity_block_3d(
        x, kernel_size=(3, 3, 3), filters=[256, 256, 1024], stage=4, block="b"
    )
    x = identity_block_3d(
        x, kernel_size=(3, 3, 3), filters=[256, 256, 1024], stage=4, block="c"
    )
    x = identity_block_3d(
        x, kernel_size=(3, 3, 3), filters=[256, 256, 1024], stage=4, block="d"
    )
    x = identity_block_3d(
        x, kernel_size=(3, 3, 3), filters=[256, 256, 1024], stage=4, block="e"
    )
    x = identity_block_3d(
        x, kernel_size=(3, 3, 3), filters=[256, 256, 1024], stage=4, block="f"
    )

    # Stage 5
    x = conv_block_3d(
        x, kernel_size=(3, 3, 3), filters=[512, 512, 2048], stage=5, block="a"
    )
    x = identity_block_3d(
        x, kernel_size=(3, 3, 3), filters=[512, 512, 2048], stage=5, block="b"
    )
    x = identity_block_3d(
        x, kernel_size=(3, 3, 3), filters=[512, 512, 2048], stage=5, block="c"
    )
